...
export default function RundownPage() {
  ...
  const checkTimeStatus = (startTimeStr: string, durationMin: number) => {
  ...
  const filteredRundown = RUNDOWN_DATA.filter((item) => {
    const status = checkTimeStatus(item.time, item.duration);
            ...
            >
              ...
              {(isLive || isUpcoming) && (
                <div className="absolute right-4 top-6">
                  <span
                    className={`text-[9px] font-black px-3 py-[2px] rounded-full border-2 italic ${isLive ? 'animate-pulse' : ''}`}
                    style={{
                      borderColor: lime,
                      color: lime,
